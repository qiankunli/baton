...
> 状态：目标设计。本文定义 Baton Plugin 的包、实例、激活边界和运行契约，作为实现
> bundled `reqloop` 前的设计基线。Loop 控制面的整体位置见
...
1. 建立 Package、Instance、Binding 和 Contribution 判别联合；用 bundled reqloop 的一个
   `/requirement` command 验证激活、关闭和多实例路由。
2. 建立 PluginResource 的通用信封与存储、同 key 不并发的 reconcile queue，以及持久化
   `requeueAfter` due time；运行数据全部归当前 BatonSession。
3. 接通 Board projection、`proposedInput` 与可选 Context projection，跑通用户审核文本后驱动
   Harness 的 Requirement Loop。
4. reqloop 出现真实外部变化需求后再接 EventSource；无法表达成 desired state 的独立命令出现
   后再接 Action，不给 Plugin 预造 Monitor 或私有 timer。
5. 真实 loop 证明必须由 Reconciler 主动启动 Harness 后，再设计受控调用；首期只允许用户把
   `proposedInput` 提交成普通 Input。
6. 出现第二个独立分发的 Plugin 后，接入 Marketplace，并落下安装源、内容信任、进程隔离和
