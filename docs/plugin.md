...
> 状态：分阶段实现。Instance 持久化、可信进程内 Package 激活、Binding 生命周期，以及
> Resource / Reconcile / Proposal / 动态唤醒已经落地；manifest、Command、Board、权限和
> Marketplace 仍按真实产品入口增量实现。Loop 控制面的整体位置见
...
当前可信进程内实现先把运行契约收窄为 `pluginId + version + activate(context)`；manifest、
配置 schema 和可安装前审阅的声明随 `/plugins` 安装流程一起落地，不先造一份无人消费的格式。
...
首期不实现 Marketplace，只开放 Installed 与 Instances：

- 查看 bundled reqloop 的版本、Contribution 和权限声明；
- 创建、配置、启用、禁用和重新激活 PluginInstance；
- 选择当前 BatonSession 的默认 Instance；
- 查看 Binding 状态和最近一次激活错误；
- 删除 Instance；bundled Package 可以禁用但不能卸载。

`/plugins` 打开统一管理界面，不要求用户记忆 `/plugins install ...` 等命令参数。Plugin 自己的
`/requirement` 等 command 用来使用领域能力，`/plugins` 只负责能力的获取、配置和生命周期。
这些管理操作由 Baton core 执行，不注册成普通 PluginContribution，也不能被 Plugin 自己
拦截或替换。
...
当前 `Manager` 从本 BatonSession 的 `instance.json` 读取启用 Instance，以精确
...
### 3.4 首期只支持可信的进程内 Plugin

首期只有随 Baton 交付的 reqloop，使用进程内 TypeScript 激活。它属于 trusted code，Baton
不宣称提供安全隔离；窄 Plugin 契约的目的首先是稳定架构边界，而不是假装沙箱。

Marketplace、第三方安装、签名、独立进程协议和版本协商等到出现第二个需要独立分发的真实
Plugin 后再落地。届时可以让进程适配层实现同一 Binding / Contribution 契约，不需要提前把
当前实现改造成 RPC。

### 3.5 目录兑现领域边界

首期目录保持两层，不为每种 Contribution 建平级子目录：

```text
src/plugin/          # Baton Plugin 领域：Package / Instance / Binding / Contribution
plugins/reqloop/     # bundled Plugin，只依赖公开 Plugin 契约
```

`src/plugin/` 是 Baton core 的 Plugin Manager 边界；`plugins/reqloop/` 拥有 Requirement Loop 的
领域模型和 Connector。Baton core 除安装注册入口外，不依赖 reqloop 的领域类型。
...
3. 以 bundled reqloop 的 `/requirement` 验证 Command 的真实需要，再补
   `command | resource` 声明校验和多实例路由，不先为未接产品入口的 Command 造 handler。
4. 接通 Board projection、`proposedInput` 与可选 Context projection，跑通用户审核文本后驱动
   Harness 的 Requirement Loop。
5. reqloop 出现真实外部变化需求后再接 EventSource；无法表达成 desired state 的独立命令出现
   后再接 Action，不给 Plugin 预造 Monitor 或私有 timer。
6. 真实 loop 证明必须由 Reconciler 主动启动 Harness 后，再设计受控调用；首期只允许用户把
   `proposedInput` 提交成普通 Input。
7. 出现第二个独立分发的 Plugin 后，接入 Marketplace，并落下安装源、内容信任、进程隔离和
   升级协议；这些能力继续由 `/plugins` 管理，不改变既有 Instance / Binding 运行模型。
