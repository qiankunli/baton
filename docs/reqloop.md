...
resourceRef、领域 ID 和 provenance 关联不同 owner 的信息。并行更新由 Baton 生成新的 Board revision，
...
使用与 Harness 权限请求相同的 Permission Gate，为每个
...
source 和可选 `parentEventId` 的 `board.changed` 事件，reqloop 据此重新读取 snapshot、评估并提出
新的 Intent。

Board observer 不能直接调用 Connector 或执行副作用；Intent 仍需经过 Baton 的
Policy/Permission/Action 路径，并使用 source、`parentEventId` 和幂等键避免 contribution
...
11. reqloop Action 与 Harness tool approval 使用同一套 Baton Permission 语义。
