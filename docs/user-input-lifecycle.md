...
概念叫 **Input** 而非 UserInput 是有意的：input 有**来源**维度（对称于 Turn 的 origin）。当前只有 `user`（composer 键入），将来会补 `monitor`（事件驱动，如监听到 PR merged 唤醒会话，见 design.md "事件驱动的长期 loop"）——monitor input 仍产生 driven turn（baton 响应外部事件主动驱动 harness），只是 source 不是用户。名字保持来源中立，`source` 字段等 monitor 真落地时再按 kernel §5 演进规则补。
...
普通输入先交给 BatonSessionRuntime。runtime 以 driven turn 全局串行：输入留在队列时可召回；一旦出队，runtime 立即把原始用户消息和 running 状态写入事件流，再准备或调用 harness。这样 harness 冷启动不会让输入在界面上消失，崩溃恢复也始终有一条完整历史。
...
- harness 尚在 preparing、尚未提交 prompt 时，runtime 立即合成 cancelled 终态；
...
| queued follow-up | runtime 全局 FIFO | harness 无感知 | harness 无感知 | 已支持；当前 turn 结束后开启下一 turn |
...
**当前结论：未支持，暂缓；但可做的切片只有一个。** query1 一出队就成为 admitted prompt。即使仍在 harness 冷启动阶段，Esc 也会留下 cancelled turn，而不会恢复 draft。只有仍在 runtime 队列中的输入可用召回操作回到 Composer。
...
- `src/session/runtime.ts`：一等 `InputRecord`（身份即 messageId + status）、`inputs` 快照、队列、active driven turn、steer 降级、cancel 与 finalize 的状态迁移；`Control` 类型 + `control(signal)`（interrupt 为首个 kind）；finalize 里 pending request 的 cancel-cascade。
