...
- request 以稳定 id 关闭，支持用户作答、用户 cancel、harness 带外解决、turn cancel、timeout 五种终态；timeout 的计时和终态决策由 Controller 持有，chat-tui 只展示 deadline/countdown，adapter 只负责把最终响应映射回 harness；
