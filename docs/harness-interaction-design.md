...
- harness runtime 先归一成共同 LLM event，再交给 session processor；request lowering 与 transport/execution 分离。Adapter 同理应把 wire 翻译集中在 harness 目录，不把 harness 分支泄漏到 runtime。
...
- request 以稳定 id 关闭，支持用户作答、用户 cancel、harness 带外解决、turn cancel、timeout 五种终态；timeout 的计时和终态决策由 baton runtime 持有，chat-tui 只展示 deadline/countdown，adapter 只负责把最终响应映射回 harness；
...
`submit()` resolve 不代表 turn 完成。用户输入的 owner 是 runtime，harness 执行过程的 owner 是 Adapter：
...
这里的 `expectedTurnId` 始终是 baton turn id。runtime 在接受输入、进入 pending 队列时就分配该 id，而不是等真正 dequeue 执行；adapter 维护 baton turn id 到 harness turn id 的映射，并在 Codex 等 wire 调用中换成 harness id。harness id 不进入 chat-tui 或 runtime action contract。
