...
用户输入不是“一次 submit”这么简单：普通 prompt、queued follow-up、same-turn steer、recall 与 interrupt 共同组成一条生命周期。当前 provider 能力、三类关键时序场景与待决边界统一见 `docs/user-input-lifecycle.md`；本节不再复制易随实现变化的状态矩阵。
...
Adapter 抽象（"小核心 + 可选能力"、`AgentAdapter` 接口、能力 descriptor 与扩展契约）是**内核定义**，见 `docs/kernel.md` §4；完整契约条款见 `docs/provider-interaction-design.md`。本节只记两家首批 provider 的原生接入细节。

各家用其最强的原生协议接入，统一发生在 baton 的 Adapter 层——tutti 验证过的路线（它曾用 ACP bridge 接 Codex，后退役换 app-server）。Provider 列表是运行时 registry，不是 core 的封闭枚举。
...
各家原生形态到统一事件的逐项映射（含运行阶段等短寿命状态）、终态收口与丢事件自愈见 `docs/provider-output-lifecycle.md` §2–§3；本节不复制易随 provider schema 变化的映射表。
...
Composer          输入框（现在时）
  ├ Provider Status  单行：driven turn > background turn > 当前输入目标 idle
...
- **Provider Status 是输入框的单行状态，不是独立层**：优先显示当前 driven turn；没有 driven turn 时显示 provider 自发的 background turn；完全空闲才回落到当前输入目标。运行相位是秒级现在时、只有当下有意义，只出现在这里、不落 transcript。多运行者并发尚未进入当前产品范围，不提前用多行表达；plan 寿命不同（turn 级、值得回看），盖棺后落 transcript 终态卡。
...
- **pin 的判断尺：只有"当前 provider 的未完成 plan + 该 provider 有回合在运行"才 pin，且 pin 带消失规则**：`[Plan]` 层仅在当前输入目标的 plan 有未完成项、且同一 provider 处于运行态时渲染（对齐 opencode sidebar / pi-mono widget 的业界惯例）。切换 provider 后，上一家未完成的 plan 卸下 pin 归 transcript；全部完成或 idle 后也同样卸下——pin 是"现在时"层，搁置即过去时，否则 provider 状态更新缺失或中途放弃时 pin 永驻，别家 provider 的回合也会误将它重新激活。切回且该 provider 重新开跑时可重新上 pin。超长 plan 窗口对准第一个未完成项，保证"现在进行到哪一步"始终可见。plan 信息**不进 Provider Status 行**——相位行要求稳定短小、每秒重绘，塞可变长步骤文本会抖动（codex / opencode / pi-mono 三家也均未这么做）；进度摘要（`plan:2/4`）归 Footer。
