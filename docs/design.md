...
| baton 自有 slash command | command registry → baton core | 不下发 provider | 不下发 provider | 已支持 `/provider`、`/model`、`/sessions`、`/new`、`/exit`；其中 `/model` 是 baton 统一的 provider 控制面 |
...
| Agent 结构化提问 | 独立的 user-input request/response（不是 permission） | `AskUserQuestion` | `item/tool/requestUserInput` | 未支持；当前 `ApprovalRequest` 不能承载多问题、多选、Other / 自由文本 |
...
2. **chat-tui picker 不等于 agent question**。现有 picker 只适合单题单选的产品命令；provider question 还可能包含多题、多选、自由文本、secret、preview 和超时。
...
4. 后续接入 structured question 时，应先在 baton 事件层建立 request/response 模型，再给 chat-tui 增加对应的展示形状；provider 原始 payload 继续放在 `raw` 中保真。
