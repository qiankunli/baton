...
| 5 | 会话数据存哪 | `~/.baton/sessions/<id>/session.jsonl`（delta + turn-summary）+ `meta.json`；它们承载 BatonSession 持久历史，原生会话只用于加速恢复 | 已定 |
...
| 11 | TUI 用什么 | opentui；运行时随之选 Bun | 已定 |
...
TUI 使用 opentui React reconciler。当前布局为 transcript、可增长 composer、状态栏与贴近 composer 的命令 / 引用 / 审批浮层；`/sessions` 提供持久会话切换，`/new` 新建会话。
