...
| same-turn steer | `Steerable` capability + `BatonSessionRuntime.steer()`（拒绝时显式降级 follow-up） | SDK 有 streaming input/control channel，安全边界语义验证前不声明 | `turn/steer(expectedTurnId)` 已映射 | Codex 已支持；Claude 未声明（自动降级 follow-up）；不能把排队 follow-up 标成 steer |
