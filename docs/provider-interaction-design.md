...
- **Request**（provider → 用户，solicited）：provider 阻塞并征求用户，`kind ∈ {permission, question(给候选/选择), hook_trust(启动信任), elicitation(取数据)}`。各 request contract 独立（下面各小节 + §4.7），不合成万能字段。
...
- 完全未知的 notification 不进入主 session timeline，进入 session 目录下与 `<runId>.jsonl` 配对的 `<runId>.log`，并记录 method/type 与计数；不能无声 `default: break`；
