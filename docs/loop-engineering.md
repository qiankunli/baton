...
> Provider 上下文接力”的实施范围。文中的 `Harness` 是现有 `Provider` 概念的目标命名；
> 具体类型、接口和迁移节奏以后续实现为准。
...
届时 Baton 放开的仍是受控的 Harness 驱动能力，而不是 `AgentAdapter`/Harness 的裸句柄：
...
causation 和 depth，默认不重新触发同一 binding，避免自激循环。
...
namespace 下的 key；跨 Plugin、跨 Harness 的关联通过 resourceRef、correlation 和派生项表达。
...
申请用户同意不是 Board 的独立职责。Baton 应提供统一的 Permission Gate，让 Harness/Provider
的 tool 请求、Plugin 的 Action、以及未来的 Harness Work 使用同一套用户授权语义。Board 只是
PermissionRequest 的一种展示和追踪入口；真正的决策、策略与回执属于 Policy/Permission。

Baton 按 `requester + operation + scope` 保存授权策略，其中 requester 可以是 Harness session
或 PluginInstance。首期至少提供：
...
```text
PermissionRequest
├── requester: HarnessSession / PluginInstance / Baton
...
Harness adapter 将现有 Provider/Harness 的 tool approval 归一到该模型；Plugin Action 不另造
一套“Board 审批”。Permission 状态可以投影进 Board，方便用户统一查看，但 BoardItem 本身不是
授权凭证。
...
当前代码中的 `Provider` 先继续服务 v1，后续向 `Harness` 迁移；持久事件和兼容层不需要随内部
类型一次性重命名。Plugin 模型则直接按新边界设计，不再引入额外的外部系统运行角色。

建议按以下顺序演进：

1. devloop 在 Harness 内提供稳定 DevelopmentOutcome；Harness 边界归一成 Baton event；
2. 建立 PluginPackage/PluginInstance、EventSink 和统一事件信封；
3. 支持 slash command、resource/query、BoardContribution 与 ContextContribution；
4. 建立 EventSubscriber、Hook、Schedule 和持久 cursor，先只观察或走 Manual 策略；
5. 接入 ActionIntent，统一走 Policy、Permission 和回执；
6. 用 reqloop + 安装了 devloop 的 Harness 跑通 Requirement Loop 示例；
7. 真实工作区证明必须由 Plugin 主动续跑 Harness 时，再实现 HarnessWorkIntent；
8. 真实需求证明离线实时触发有必要时，再引入 daemon。
...
12. Harness/Provider 与 Plugin 共用 Permission 语义；Board 只投影请求和结果，不持有授权。
...
2. Plugin Package、Instance、workspace 和用户配置的作用域及继承关系是什么？
3. Event ack、cursor、回放、乱序和 schema version 如何定义？
4. PermissionPolicy 的 scope 继承与覆盖规则如何跨 requester、operation、workspace、环境和
   session 表达？
5. 哪些真实场景足以触发 HarnessWorkIntent 的实现？实现后与用户 turn 冲突时如何排队、
   steer 或取消？
