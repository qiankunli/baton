...
- **approver（审批人）**：Codex 默认由自动 reviewer 审批，用户可显式切回 TUI 人工审批。默认委托不等于静默：当前授权方必须全局可见，每次自动决定必须有权威回执。
...
| 审批选项 | `PermissionOption[]`（含 `always`） | 仅当 SDK 给出 permission suggestions 才提供 `always` | `availableDecisions` | 已支持；baton 按 provider 实际候选映射，不自造 always |
| 权限模式 | 尚无统一入口 | SDK 有 default / acceptEdits / plan / bypassPermissions | `approvals_reviewer` + sandbox 策略 | 未统一暴露；各家模式尚未进 baton 配置面 |
| 自动审批 / 委托 | 默认委托 + 权威回执 | 无逐条 reviewer；acceptEdits / bypass 是另一形态的委托（模式而非 reviewer） | `approvals_reviewer="auto_review"` + `item/autoApprovalReview/*` | Codex 默认开启；事件 UNSTABLE |
...
### 2.2 默认 `reviewer=auto_review` + 人工审批开关

Codex adapter 默认在启动参数注入 `-c approvals_reviewer="auto_review"`；配置 `codexApprovalReviewer: user` 可切回 TUI 人工审批。若 `codexCommand` 已写 `approvals_reviewer`，则命令级取值优先，Agent Status 也按实际生效值展示。

人工审批时，adapter 优先按 app-server 的 `availableDecisions` 生成选项并把结构化 decision 原样回传；只有老版本未提供该字段时才退回稳定四选项。这样不会把“本次允许”“本 session 允许”和“写入命令/网络规则”混成同一个按钮。

### 2.3 未知审批旁路的兜底

正常 auto-review 由 §3 的权威回执留痕。若某个 item 变成 `declined`，却既没有向用户发过 requestApproval，也没有收到 auto-review 回执，adapter 会启发式 emit `_baton_notice`（“Approval bypassed by provider-side policy”）。它只用于未知或旧版 provider 策略的悲观对账，不与权威回执重复。
...
- **默认委托、可撤回**：默认 `auto_review`；`codexApprovalReviewer: user`（或命令级显式覆盖）切回人工审批。
...
auto-review 开启时，Agent Status 常驻 `approvals:auto-review`——让用户**随时**知道审批权已委托，而不是只在逐条回执里被动发现。委托是显式且持续可见的。
