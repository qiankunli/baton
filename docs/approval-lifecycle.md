...
Codex adapter 默认在启动参数注入 `-c approvals_reviewer="user"`；配置 `codexApprovalReviewer: auto_review` 才显式委托。若 `codexCommand` 已写 `approvals_reviewer`，则命令级取值优先，Agent Status 也按实际生效值展示。
