...
- **approver（审批人）**：谁来批。baton **不替 provider 定这个默认**——缺省跟随 provider 自己的解析（codex 自身默认是 `user`），用户可显式覆盖。无论谁批，当前授权方必须全局可见，每次自动决定必须有权威回执。
...
| 自动审批 / 委托 | 跟随 provider 默认 + 权威回执 | 无逐条 reviewer；acceptEdits / bypass 是另一形态的委托（模式而非 reviewer） | `thread/start.approvalsReviewer` + `item/autoApprovalReview/*` | Codex 自身默认 `user`；事件 UNSTABLE |
