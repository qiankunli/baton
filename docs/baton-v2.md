...
> 不代表这些对象已经实现，也不改变当前 v1 以统一会话和跨 Harness 上下文接力为主的范围。
...
- **FactEvent**：已经发生、会改变 reducer 或领域判断的事实。它先持久化，再分发，可按序列和
  cursor 重放；
- **InvalidationSignal**：提示“可能有新状态”的唤醒或失效信号。它可以合并、重复或丢失，
  消费方收到后读取权威快照，再把观察到的新事实写成 FactEvent。

二者可以共享 correlation、causation 和 provenance 等信封字段，但不能共享可靠性承诺。
`wake(sessionId)`、文件变更通知和投影刷新属于 signal；用户输入已 admit、外部资源状态改变、
Harness 接受工作和 Action 完成属于 fact。

```text
signal ──▶ read canonical state ──▶ append FactEvent ──▶ reduce / project
                         ▲
FactEvent source ────────┘
```
...
### 1.4 Intent、Attempt 与 Receipt

所有可能跨进程、调用外部系统或唤醒 Harness 的工作，都采用同一个三段模型：

```text
FactEvent / user decision
          │
          ▼
Intent ──▶ Attempt ──▶ Receipt
           │
           └── prepared → dispatching ─┬→ accepted → settled
                                      ├→ uncertain → accepted / settled
                                      └→ settled（确认未接收）
```
...
harnessSessionId?`，并带 correlation / causation。Baton 先持久化 Attempt，再发起外部投递；
...
存在完全相同的生命周期，core 只提供 ResourceRef、FactEvent、Intent/Attempt/Receipt、
correlation 和 projection 原语。
...
1. runtime 预分配稳定 Input/Turn identity，并持久化 admit 事实；
...
同一 BatonSession 的 driven Turn admission 继续由 coordinator 串行化；不同 BatonSession
...
外部变化 / Schedule
  ...
  → accepted | uncertain
  → Receipt / result FactEvent
  → Board projection + Plugin 下一次决策
```

Schedule 和 Hook 只产生事实、contribution 或 Intent，不拥有自己的执行状态机。进程恢复时，
Baton 扫描未 settled 的 Attempt 和各 subscriber cursor；它不依赖某个 timer callback、
内存队列或 wake 信号仍然存在。
...
3. 扫描未 settled Attempt：`prepared` 可继续投递，`accepted` 查询进度，
...
6. 最后发出必要的 signal，让 runtime 拉取并推进仍有工作的稳定 ID。
...
signal 提醒 coordinator 按 ID 取任务；signal 合并或丢失时，恢复扫描仍能找到工作。可靠性由
ledger 和幂等投递提供。
...
2. 明确 FactEvent 与 InvalidationSignal 的不同承诺；
3. 先用 ActionIntent 验证 Intent/Attempt/Receipt 和 `uncertain` 对账；
