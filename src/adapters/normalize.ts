...
/**
 * 封闭词表归一（kernel.md §2 不变量 #2 的单一结构保证）：把 provider 的开放 / UNSTABLE
 ...
 * 结构保证；新 provider 接入时免再重新发明这条纪律。归一发生在 adapter 边界：闭合值进入
 * 事件流后，reduce / 投影都面对闭集，不必再兜未知；provider 原始值仍在信封 `raw` 里保真。
 *
 * @param raw provider wire 上的原始终态值（容忍 unknown 类型）
