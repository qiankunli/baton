...
// effort 优先；这里仅让新 BatonSession / 新进程沿用各 provider 最近一次显式选择。
...
export function loadEffortPreferences(rootDir?: string): Record<string, string> {
  ...
  try {
    ...
    return Object.fromEntries(
      Object.entries(persisted.efforts).flatMap(([provider, effort]) =>
        provider.trim() && typeof effort === "string" && effort.trim() && effort !== "default"
          ? [[provider, effort] as const]
          : [],
      ),
    );
  } catch {
...
/** `default` 表示重新跟随 provider，因此删除该 provider 的持久偏好。 */
export function saveEffortPreference(rootDir: string, provider: string, effort: string): void {
  const efforts = loadEffortPreferences(rootDir);
  if (!effort || effort === "default") delete efforts[provider];
  else efforts[provider] = effort;
