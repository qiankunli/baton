// /model 是用户级运行偏好，不改写手工维护的 config.yaml。按 provider 记住最近一次
...
export function loadModelPreferences(rootDir?: string): Record<string, string> {
  ...
  try {
    ...
    return Object.fromEntries(
      Object.entries(persisted.models).flatMap(([provider, model]) =>
        provider.trim() && typeof model === "string" && model.trim() && model !== "default"
...
export function saveModelPreference(rootDir: string, provider: string, model: string): void {
  const models = loadModelPreferences(rootDir);
  if (!model || model === "default") delete models[provider];
