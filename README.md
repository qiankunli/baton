...
Set `codexApprovalReviewer: auto_review` to delegate Codex approvals to its risk reviewer. Baton keeps the delegation visible in the footer and records each automatic decision beside its target tool; the default `user` keeps interactive approval cards in Baton.
