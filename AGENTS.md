...
- **BatonSession、HarnessTarget 与 HarnessSession 分属三层**：BatonSession 是用户拥有、跨 harness、可持久恢复的逻辑历史；HarnessTarget 是具体执行与状态实例坐标；HarnessSession 只是该 Target 启动出的私有执行状态。Controller slot、原生 session、同步水位和 Target-scoped 投影状态一律按 `harnessTargetId` 隔离，Harness 只描述协议 / Adapter 类型。driven turn（用户 submit）在 BatonSession 内全局串行；harness 自发的 observed turn 与队列正交，见 `docs/kernel.md` §3。
