...
- **Plugin 通过只读 Builtin Resource 感知 Baton 内部事实**：Builtin Resource 是 Event Ledger 的 replay/live 投影，不另建真相也不允许 Plugin patch；需要 desired/observed state 时使用 Plugin 自有的 `spec/status` Resource。两者可共用 keyed reconcile，但 Plugin 不必先创建 Resource、更不必是 loop。
