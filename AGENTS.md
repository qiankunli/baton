...
- **用户输入的 owner 是 runtime，provider 执行的 owner 是 Adapter**：driven turn 出队即由 runtime 落 `user_message`/`running`（原始输入进正典历史，provider 冷启动不阻塞 Transcript，preparing 可取消）；Adapter 只报告执行过程与终态，steer 成功时补 `delivery:"steer"` 的用户消息。见 `docs/provider-interaction-design.md` §4.1。
